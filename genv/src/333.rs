...
impl Expression {
    pub fn evaluate(&self, context: &Context) -> Result<Expression, String> {
        // Updated to use context with RefCell
        match self {
            ...
            Expression::Function(func) => {
                context.set_function(func.name.clone(), *(*func).clone()); // Unbox and insert
...
fn main() {
    ...
    println!("{:?}",single_line_function);
    // Define a multi-line function
    // let multi_line_function_code = "
    //     multiply(x, y) =>
    //         {
    //             z = x * y
    //             z
...

    {
        ...
        let add_args = vec![
            Expression::Atom(Atom::Integer(5.0 as i64)),
            Expression::Atom(Atom::Integer(3.0 as i64)),
        ];
        let mut local_context = context;

        println!("{:?}",add_function);

        let result = add_function.call(add_args, &mut local_context).expect("Function call failed");
    ...
    //     ];
    //     let result = add_function.call(add_args, &mut context).expect("Function call failed");
    //     println!("Result of single-line function call: {:?}", result);
    // } // End of scope for immutable borrow
    //
    // // Test multi-line function
    // {
    //     let multiply_function = context.get_function("multiply").expect("Function 'multiply' not found");
    //     let multiply_args = vec![
    //         Expression::Atom(Atom::Integer(4)),
    //         Expression::Atom(Atom::Integer(7))
    //     ];
    //     let result = multiply_function.call(multiply_args, &mut context).expect("Function call failed");
